...
元数据取值——`类型`、`控件`、`顺序`、`RID`、`标识`、`列宽`、`主键`、`关键字段`、锚点后缀——的唯一权威是 `references/client-source-contract.md` 第 9 节。新生成元数据中物理 `date`/`datetime`/`datetime2` 用 `类型='D'`、其余用 `类型='S'`，普通字段 `控件='E'`、`*_PJLX` 用 `控件='S'`；不得从 bit、精度、标签或布局推断其它取值。其它 reference 与本文件都不复述完整取值表，出现分歧时以第 9 节为准。
