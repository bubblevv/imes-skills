...
1. **键不能混用。** 先读取当前 `v_tbcolumn` 定义：在本客户端的元数据投影路径中，`SYS_TbColumn.GLZD` 通过 `IOJCBDZD.IOJCBDZD_Vkey` JOIN 到参照路由（通常还要求 `IOJCBDZD_TOP=1`）；`IOJCBDZD_MC` 是 `AutoOpen`/直接业务表单路由的可见名；字段 `帮助` 值查 `IOJCBDZD.IOJCBDZD_BZBH`；单据路由查 `IOBDZD.IOBDZD_MC`（审批路径查 `IOBDZD_BH`）。四个消费者独立，键写错通常只返回空串。
...
6. **控件码只能来自当前版本源码 + 同库工作单据，不能按物理列类型推断。** 表头 `控件='S'` 是静态文本、`'C'` 是复选框；`类型='D'` 是**只读日期格式框（右键日历）**、`'DT'` 才是日期时间选择器、`'EM'` 才是多行框。CBill 新增清空路径只有在日期字段 `类型='D'` 且 `置空=1` 时才会写入 `AppData.adddate`；该值由登录窗口初始化为账务日期，不能把 `SYS_TbColumn.默认值` 当成日期初始化开关。若业务要求自然日，必须同时核对登录账务日期与物理列 `GETDATE()` 默认兜底。
...
4. Read active `BDJB` for the visible form name. The four standard lifecycle rows (审核/取消审核，各自前置检查和执行行) must be present and executable; verified feature rows for inventory, source-document checks, downstream backfill, or asset lifecycle are allowed and must be audited separately. Zero active rows is an initialization failure; do not inspect disabled scripts or referenced procedures until this check passes.
...
### Live 元数据全量门禁（动态单据改动后强制执行）

静态脚手架校验不能证明目标库已经满足运行时契约。只要修改了 `SYS_TbColumn`、`IOBDZD`、`IOJCBDZD`、`BDJB`、`SYSWSPACE`、`sysmenu` 或主从单据物理字段，就必须在已核对身份的目标库执行 `scripts/audit_live_dynamic_bills.sql`。该脚本从目标库现存 `IOBDZD` 自动发现 EAM/设备动态单据，禁止只审计本次生成脚本里手工列出的表单。

- 结果包含任一 `FAIL` 时，交付状态必须是 `review-blocked`，不得继续部署、不得以 `admin` 能打开代替检查，也不得只报告首个失败项；先记录所有表单和失败码，再按最早运行时失败修复。脚本默认输出报告结果集，不依赖末尾异常来传递失败；`AuditStatus=FAIL` 本身就是阻断信号。
- 审计必须覆盖路由/物理表键、维护页和独立查询页、字段闭合、顺序、`NULL` 与空字符串区别、`GLZD/LMark/RMark` JOIN、固定别名、日期初始化、BDJB 基础生命周期、工作区和七个标准 `sysmenu`。
- 日期门禁必须分成两层：元数据 `类型=D/置空=1` 只证明客户端 `SetBlank` 会把登录账务日期写入输入框；物理 `date/datetime/datetime2` 的 `GETDATE()` 默认值只是保存时兜底，不能证明界面显示的是自然日。发生“新增日期不是当前日”时，必须同时读取登录路径 `AppData.adddate`、账务日期和物理默认值。
- `PO=2` 维护页明细的 `<DETAIL>_SJDH` 显示名和标签名必须是 `单号`，因为 `PageRowsTab` 外层固定 `WHERE 单号=...`；查询页同一物理字段通常必须用 `明细单号` 去重，二者不能混用。
- `BDJB` 不使用“有效行数必须恰好 4”作为总量门禁：四条基础审核/撤审规则必须逐条存在，额外业务校验和回填规则必须逐条检查其目标、顺序、前置条件和反向路径。
- 只有 live audit 全部通过后，才执行维护页、查询页、翻前单/翻后单和新增保存的 Release 冒烟；数据库等价 SQL 通过不能冒充真实客户端重开。脚本修订后必须先做脚本自测、再在目标库执行一次完整审计；审计器自身编译/汇总错误也属于阻断，不得把“脚本跑完”当成“表单通过”。
...
4. Run `scripts/validate_dynamic_bill_artifacts.py` on the final `forward.sql` and `crud-test.sql`. It resolves each `SYS_TbColumn` tuple against the statement's actual column list, checks arity, `标识=NULL`, physical date fields using `类型=D` and other new fields using `类型=S`, ordinary `控件=E`/`*_PJLX` `控件=S`, visible-label minimum `列宽`, `PO=1` 运行锚点后缀（`_ID/_SJDH/_YWRQ/_PJLX/_SHBZ/_ZDR/_SHR/_ZY`）的唯一性与结尾匹配，`GLZD` 非空行的 `LMark/RMark` 不得为 `NULL`，fixed or `@RIDBase+offset` RID uniqueness/range, the required `MAX(RID)+1` declaration and positive-value guard, CRUD column/value parity, explicitly supplied audit coordinates, same-row outer-rectangle collisions, and help-bearing widths against the measured browse-button footprint. `--expected-audit-gap` applies only when `SHBZ` is wrapped below a prior row; a same-row audit is valid without that vertical gap. It accepts both `INSERT dbo...` and `INSERT INTO dbo...`, normalizes quoted SQL literals, and excludes the separate `<MC>查询` layout from maintenance-page audit-coordinate assertions before SQL Server is involved. 它只检查静态产物，不能替代 `references/client-source-contract.md` 第 7 节清单里需要连接目标库才能证明的项（键唯一性、排序规则、`PRD_GETDANHAO`、权限行）。
...
- A module-wide suffix migration such as `..._BH -> ...BH` is never a two-field patch: inventory the entire module, carry the mapping through physical columns, foreign-key/index/constraint definitions, `SYS_TbColumn.字段名/GLZD`, `IOJCBDZD` routes, **`BDJB_SQL` and the `IOYYGX`/`IOPOPDLG` relation SQL**, current scripts and ER artifacts, and keep old names only in labeled migration history. Require old/new field counts, metadata impact counts, property preservation, foreign-key closure, runtime JOIN/help replay, exact row-count assertions, and a guarded reverse map before deployment. After deploying a rename, run `scripts/audit_live_dynamic_bills.sql` with `@MigratedColumns` populated from the old-to-new map; the audit sweeps every live dependency carrier, including `BDJB_SQL`, so a stale reference cannot survive in a carrier that no hand-written check named.
...
Only update this personal skill when the user explicitly requests or authorizes it. First record project-specific evidence in the active repository's `docs/imes/` or `diagnostics/` layer. Then add only a concise reusable entry to `references/patterns.md`: symptom, likely cause, safe fix pattern, and verification. If the new rule changes task routing or a first-action gate, update this `SKILL.md` and the applicable `references/evaluation-cases.md` as well. Exclude customer names, proprietary customer rules, document numbers, credentials, dated incidents, and unverified schema assumptions. A live-audit script is itself a runtime artifact: after changing it, compile/execute the full script against the resolved target, inspect every result set, and add a regression assertion for the changed route/date/failure-reporting rule before treating the skill update as complete.
