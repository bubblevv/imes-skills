...
可能原因：这些错误几乎都不是数据库缺陷，而是编译进 EXE 的常量没有被满足——空的 `显示名/字段名`、`标识` 被填成 `'0'`、`GLZD` 行的 `LMark/RMark` 为 `NULL`、`PO=1` 某行 `控件/类型/RID/显示/必填/只读` 为 `NULL`、`单号`/`分录号` 别名缺失、`分录号` 不能 `cast as int`、可见名在 `IOJCBDZD/IOBDZD/SYS_TbColumn/SYSWSPACE/sysmenu` 之间不一致、元数据视图把 `GLZD` 错当成 `IOJCBDZD_MC` 而不是当前视图实际使用的 `IOJCBDZD_Vkey`、帮助键 `IOJCBDZD_BZBH` 被混用、库排序规则为大小写敏感。源码读不到的键只返回空串，空串被直接拼进 SQL，所以没有异常、没有堆栈。
...
症状：主从动态单据新增时日期框为空、显示的日期不是自然日，或不同单据表现不一致；只检查 `SYS_TbColumn.默认值` 看不出原因。

根因判定：活动客户端 `SetBlank` 只有在元数据 `类型='D'` 且 `置空=1` 时才把 `AppData.adddate` 写入控件；登录窗口把账务日期赋给 `AppData.adddate`。因此界面显示值来自登录账务日期，不来自 `SYS_TbColumn.默认值`，也不直接来自 SQL Server 当前时间。物理日期列的 `date/datetime/datetime2 NOT NULL DEFAULT GETDATE()` 是保存层兜底，不能替代界面初始化。若表头日期物理列是 `nvarchar`、可空或无默认值，属于字段契约缺陷；若物理契约和元数据均通过但界面日期不对，应继续核对登录账务日期和实际新增路径。

安全修复模式：先分开记录登录账务日期、`AppData.adddate`、维护页日期元数据和物理列默认值；不要把 `默认值` 字符串当作客户端初始化开关。批量修改元数据后，必须执行自动发现的 live audit，检查所有设备动态单据，而不是只检查本次变更的单据。

验证：逐单据检查物理日期类型/可空性/`GETDATE()` 默认、维护页 `类型=D/置空=1`、查询页日期锚点，并要求真实客户端用已知账务日期新增一次。数据库审计通过但没有真实新增冒烟时，只能报告为数据库契约通过。

## 批量快速请求不能跳过全量 live gate

症状：用户要求“批量设置为 1、快一点、不要测试”，后续仍有单据出现 `FROM )`、翻前单“单号”无效、日期不显示或控件初始化失败。

根因：skill 的脚手架和静态校验只是生成/解析工具，不会自动连接目标库，也不会阻止人工绕过 live audit；如果任务流程没有把全量审计作为硬性完成条件，局部脚本成功会被误当成模块完成。

安全修复模式：批量操作可以减少往返，但不能跳过目标身份核对、只读 live audit、所有失败项汇总和最小修复范围确认。每次动态单据元数据/物理字段变更后必须执行 `scripts/audit_live_dynamic_bills.sql`，以 `AuditStatus` 作为阻断信号；审计器自身的 SQL 编译错误、汇总错误或结果集不可读也必须标记为 `review-blocked`。

验证：审计范围从现存 `IOBDZD` 自动发现，记录路由数量、通过/失败单据、失败码和失败行；只有全量 `PASS` 后才进入真实客户端维护页、查询页、翻前单和新增保存冒烟。不要只报告首个失败，也不要用 `admin` 能打开代替非 ADMIN 角色权限验证。

## 物理字段改名必须扫配置表里的 SQL 文本

症状：物理列改名并同步了 `SYS_TbColumn`、`IOJCBDZD`、外键和存储过程后，单据仍报 `Invalid column name '<旧列名>'`；审核/撤审、参照或上下游功能失败，而常规元数据检查全部通过。

可能原因：改名的依赖载体不止物理列和模块对象。`BDJB_SQL` 把审核/撤审规则作为 SQL 文本存在配置表里，`IOYYGX_GLTJ/SYLJ/XYLJ`、`IOPOPDLG_JOINSTR` 同样如此。这些都不在 `sys.sql_modules` 的覆盖范围内，所以按“物理列 + 元数据 + 模块”三类做完迁移，配置表里的旧列名会完整保留。这类残留不会在部署时暴露——只有单据真正走到审核、参照或上下游分支才报错，通常由用户在几天后反馈。另一个变体是单据或明细表整体改名后，`BDJB_SQL` 里带前缀的字段名（如 `<旧表名>_SJDH`）没有跟着改。

安全修复模式：把“配置表内的 SQL 文本”作为独立的依赖类别写进迁移清单，与物理列、元数据、外键、模块并列。迁移前对每个旧名在 `BDJB`（含停用行也要记录）、`IOYYGX`、`IOPOPDLG` 上做命中计数，迁移后断言为 0。不要依赖人工逐条检查——用 `scripts/audit_live_dynamic_bills.sql`：把 old-to-new map 填进 `@MigratedColumns` 后运行，它会在所有依赖载体上做通用扫描；不填该表时还会退化为“标识符存在性扫描”，把 `BDJB_SQL` 里任何当前库不存在的、形如字段名的标识符报出来，因此连历史上已经漏掉的改名也能发现。

验证：断言旧名在 `BDJB_SQL`、`sys.sql_modules`、`v_tbcolumn.字段名/GLZD/帮助`、`IOYYGX`、`IOPOPDLG` 上命中数为 0；新名在这些载体上完整出现；审核、撤审、参照、上下游四条路径各自回放一次真实 SQL。`AuditStatus=FAIL` 必须阻断部署，不能只看“脚本执行成功”。

## BTYPE=1 新增后保存不上（静默失败）

症状：`BTYPE=1/UForm1` 基础资料新增后点保存，**没有报错、没有提示、界面无变化、数据不入库**。用户描述通常是「保持不上」「保存没反应」。

可能原因：`UForm1.cpp::CUForm1::OnTysave` 写死按 `FID` 定位身份列：

```cpp
t = m_grid.GetColPos("FID");
if (t < 0) return;          // 静默返回，整个保存被跳过
```

身份列（`顺序=0` 那一行）的 `显示名` 不是 `FID` 时，FlexGrid 里没有该列，`GetColPos` 返回 `-1`，`OnTysave` 直接 `return`。因为没有任何错误分支，界面不会有任何反馈。常见诱因是把身份列写成 `HID`——那是**动态单据 `PO=1` 表头**的约定，与 `UForm1` 无关，两套规则不能混用。

安全修复模式：把该 `表名` 下 `顺序=0` 那一行的 `显示名`/`标签名` 改为 `FID`。该列是隐藏身份列（`显示=0`），改名不影响用户可见界面。只改这两列，不动 `字段名`、`顺序`、`主键`、`RID`、`类型`、`控件`。改前先断言旧值恰为当前别名，并确认网格内没有另一个 `FID` 列（会冲突）。
...
症状：新增保存报 `将 varchar 值 '0.00' 转换成数据类型 int 时失败`（或 `Error converting data type varchar to numeric` 出现在 int 列上）。字段是 `int`，`类型` 看着也是数值类 `N*`，不像配错。

可能原因：`N` 后面不带数字时精度是 **0**，源码按 `%.<iWS>f` 格式化，写出的是 `0`；而 `NZ`/`ND`/`NJ`/`NB`/`N2` 的 `iWS=2` 写出 `0.00`，`NF` 写出 `0.0000`。**`int` 列拒绝带小数点的字面量**：`CONVERT(int,'0.00')` 直接报错，`TRY_CONVERT` 返回 NULL。反过来，`decimal` 列用 `N`（精度 0）虽然能插入，但会静默丢掉小数位。

安全修复模式：按物理类型分两组取值——整数列（`int`/`bigint`/`smallint`/`tinyint`）只能 `N` 或 `N0`；小数列（`decimal`/`numeric`/`money`/`float`/`real`）按业务语义取 `NZ`（数量）、`ND`（单价）、`NJ`/`NB`（金额）、`NF`（发票）等带小数位的值，**不要用 `N`**。改之前先用 `sys.types` 把两类列分开统计，不要对"数值列"下统一结论。

验证：按 `类型` 的实际精度算出空单元格的字面量（`N`/`N0`→`0`，`NZ`/`ND`/`NJ`/`NB`/`N2`→`0.00`，`NF`/`N5`→`0.00000`），再用 `TRY_CONVERT` 转成该列的物理类型，全部非 NULL 才算通过。用户定义类型（`sys.types.is_user_defined=1`）要取其底层 `system_type_id` 对应的系统类型再转换，否则 `TRY_CONVERT` 报"类型 xxx 不是已定义的系统类型"。

注意：`打印次数`、`审核标志` 这类由客户端内部写入、不会为空的 int 列，配 `S` 不会触发问题（`''` 分支走不到）；排查时优先看用户可输入、可为空的数值列。

## 取消审核必须严格互逆于审核

症状：取消审核后下游状态与审核前不一致；或者同一单据「审核→取消审核→再审核」得到不同结果；或者一张单据取消审核后，另一张单据的数据被改坏。

可能原因：把取消审核写成了"根据当前状态重算"，而不是"撤销这一张单据的影响"。典型写法是取消审核时去查"还有没有别的生效单据"再决定写什么——这引入了审核时并不存在的判断分支，一旦历史数据或人工改库破坏了前提，结果就与审核前不一致。另一种是把审核写本单的值、取消审核却去取"最新一张单的值"，两者本就不互逆。

正确理解：**审核 = 单据生效**，把本单影响写入下游；**取消审核 = 单据失效**，把审核写过的东西**原样撤回**，即 `cancel(audit(x)) == x`。判断取消审核写什么，看审核写了什么，逐项反过来：置 1 的置回 0、写入的值还原、清掉的值恢复、插入的行删除、累加的减回。

只有在**审核本身写的就是派生聚合值**（按所有生效单据求和）时，取消审核才需要重算——因为此时"撤销本单影响"恰好等价于"用剩余单据重新求和"。这是 `audit-state-matrix.md` 数量矩阵的范畴，不是默认写法。

安全修复模式：先找**审核的校验是否已排除多张单据同时生效**。例如借用审核校验「同一资产不能重复借出」，该不变式成立就意味着任一资产最多一条活跃单据，取消审核时不存在"还要保留别的单据"的情况，**简单互逆即可**。若审核没有这类校验，先补校验，不要用重算去兜住本可由校验避免的歧义。同时确认执行顺序：`QZJC=0`（置 `SHBZ`）排在 `QZJC=1`（业务）之前，取消审核的业务规则运行时本单 `SHBZ` 已是 0，不会把自己算进"已审核单据"。

验证：断言**互逆性**而不是固定值——记录基线（审核前状态）→ 执行审核并断言变为生效态 → 执行取消审核并**断言逐字段回到基线（含 NULL）** → 再次审核并断言与首次完全一致（幂等）。第 3 步是核心断言；只断言"取消审核后等于某个期望值"可能掩盖非互逆的实现。
