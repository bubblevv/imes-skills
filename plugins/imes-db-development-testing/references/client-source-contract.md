...
本节只覆盖 `PO=1` 表头。`左坐标`/`顶坐标`/`宽度`/`高度` 也只在这里被消费；明细 `PO>1` 与 `BTYPE=1` 走 `LoadGridSet`，不读这四列，见 9.6.9。
...
- **【硬规则 5.5】** 明细单元格行为由 Grid 字段决定（`类型`、`帮助`、`管道字符`、`只读`、`列宽`、关联映射），`控件` 在明细里只是保留字段，不是编辑器开关。**明细的 `左坐标`/`顶坐标`/`宽度`/`高度` 同样是保留字段**：`LoadGridSet` 不读它们，明细的布局契约只有 `顺序` + `列宽`（见 9.6.9）。
...
| 新增保存报 `从 varchar 转换为 numeric 时出错` | 数值物理列配了 `类型='S'`，空单元格写成 `''` | `sys.types` 里的数值列，逐一核对其元数据 `类型` 是否为 `N*` |
| 新增保存报 `不能将值 NULL 插入列`（该列是 bit） | `bit NOT NULL` 标志列配了 `类型='S'`，空值写成 `''` | `bit` 且 `is_nullable=0` 的新增列，`类型` 是否为 `C`/`R` |
| 新增保存报 `列名无效` | 元数据 `字段名` 在物理表不存在（常为漏写/错写下划线） | 元数据字段名逐个回 `sys.columns` 核对真实拼写 |
...
**默认取值由物理列类型决定，这是硬规则，不是偏好：**

| 物理列类型 | `类型` 默认 | 说明 |
|---|---|---|
| 字符类（`varchar`/`nvarchar`/`char`/`nchar`/`text`） | **`S`** | 文本 |
| **整数类**（`int`/`bigint`/`smallint`/`tinyint`） | **`N`** | 精度必须为 0，见下 |
| **小数类**（`decimal`/`numeric`/`float`/`real`/`money`） | **`N`** | 小数位按业务语义选 `NZ`/`ND`/`NJ`… |
| 日期类（`date`/`datetime`/`datetime2`/`smalldatetime`） | **`D`** | 日期 |
| `bit` 且语义是**标志/状态**（`*_BZ`、`*_ZT`、完成标志、归还标志、审核标志、启用、是否…） | **`C`** | 复选框 |
| 其它未提到的 | **`S`** | 兜底 |

**`N` 后面不带数字时小数位是 0，写出的字面量是 `0` 而不是 `0.00`。** 这一点决定成败：

| 写入字面量 | `int` 列 | `decimal` 列 |
|---|---|---|
| `0`（`N` / `N0`，`%.0f`） | **接受** | 接受 |
| `0.00`（`NZ`/`ND`/`NJ`/`NB`/`N2`，`%.2f`） | **拒绝** | 接受 |
| `0.0000`（`NF`，`%.4f`） | **拒绝** | 接受 |

`CONVERT(int,'0.00')` 报「将 varchar 值 '0.00' 转换成数据类型 int 时失败」。
所以**整数物理列只能用 `N` 或 `N0`**，用 `NZ`/`NJ`/`ND` 等带小数位的类型会让新增保存失败。
反过来，`decimal` 列用 `N`（小数位 0）虽然可插入，但会丢小数——金额/数量应按业务选带小数位的类型。

`bit` 列**不一律是 `C`**：只有语义为标志/状态的才用 `C`。`bit` 作为普通数值开关且界面按文本处理时可用 `S`。判定依据是字段语义，不是物理类型本身。

**取值不是默认值时**（例如需要指定小数位、需要日期时间选择器），用下列特化值：
...
`N*` 精度映射（源码 `iWS` 决定 `%.<iWS>f`）：`NF`=发票精度、`NB`/`NJ`/`N2`=2、`NW`=4、`ND`=单价精度、`NZ`=数量精度、`N1..N6`=定点 1..6 位，**`N` 与 `N0` 都是 0 位**（写 `0`）。

**为什么数值列必须用 `N*` 而不是 `S`：**

`GenInsertSqlFromGrid` 按 `类型` 决定生成的值字面量。`类型='S'` 时源码执行
`strtmp1.Format("'%s'", tmp)`——空单元格会写成 `''`。数值列收到 `''` 直接报
**从数据类型 varchar 转换为 numeric 时出错**，新增保存失败。`类型='N*'` 时源码走
`atof` + 精度格式化，空值写 `0.00`，不会失败。这是"新增保存就失败"最常见的原因。

- **【硬规则 9.1.1】** 按上表由物理列类型取默认值。数值列必须是 `N*`，字符列 `S`，日期列 `D`，标志类 `bit` 列 `C`。
- **【硬规则 9.1.5】** **整数物理列（`int`/`bigint`/`smallint`/`tinyint`）只能用 `N` 或 `N0`**；用带小数位的 `NZ`/`ND`/`NJ`/`NB`/`N2` 等会写出 `0.00`，`int` 列拒绝该字面量并报转换失败。小数列反过来不能用 `N`（会丢小数）。
...
- **【硬规则 9.1.4】** 数值物理列配了 `类型='S'` 是**真实缺陷**，会让该表单新增保存失败。排查"新增保存报 numeric 转换错误"时，先按 `sys.types` 找出所有数值列，逐一核对其元数据 `类型` 是否为 `N*`。
...
- **【硬规则 9.6.9】** 几何四列 `左坐标`/`顶坐标`/`宽度`/`高度` **只对 `PO=1` 表头控件生效**，明细行与网格不消费它们。
  - 唯一消费者是 `SGSoft.cpp::CSGSoftApp::CreateDlgItemWithArrayDateTime`：它逐行 `GetCollect("左坐标"/"顶坐标"/"宽度"/"高度")` 拼 `CRect`（`rect1.left/top` 直接取值，`rect1.right=left+宽度`，`rect1.bottom=top+高度`），四列全为 `NULL` 时回退到默认 `(40,60,100,80)`；`Bill.cpp` 只把**表头**记录集交给它。
  - 网格路径 `SGSoft.cpp::CSGSoftApp::LoadGridSet` 的列循环逐行只取 `顺序`（直接当列下标 `a`）、`列宽`（`grid->SetColWidth(a, iwidth==0?100:iwidth)`）、`管道字符`（`SetColComboList`）、`显示` + 角色列（`SetColHidden`）、`只读`、`帮助`、`合并`、`对齐`、`类型`；**整段没有出现 `左坐标`/`顶坐标`/`宽度`/`高度`**。
  - `LoadGridSet` 同时服务两类网格：`BTYPE=1` 表单（`iWhichPart=0`，按 `表名` 取该表全部行）与动态单据明细页（`iWhichPart=PO`）。
  - 结论：明细行（`PO>1`）与 `BTYPE=1` 的布局契约**只有 `顺序` + `列宽`**。坐标值是惰性数据——不参与渲染、不能作为布局或整版证据、不能据其“相同/重叠”判定元数据缺陷。明细能否正常显示，只看 `顺序` 是否 `1..n`（`BTYPE=1` 为 `0..n-1`）连续、`列宽`（`0` 被回退成 `100` 会静默截断）、`类型`、`显示` 与角色列。
