...
> **取值权威声明**：本文件是 `BTYPE=1` 的**流程层**（契约卡、三阶段门禁、执行格式）。元数据取值——`类型`、`控件`、`顺序`、`RID`、`标识`、`列宽`、`主键`、`关键字段`、锚点后缀——的唯一权威是 `references/client-source-contract.md` 第 9 节。`BTYPE=1` 的 `顺序` 基址差异见第 9.3.2 节（`0..n-1`，不是动态单据的 `1..n`）。两处若出现分歧，以 `client-source-contract.md` 为准，并应修正本文件的引用。
...
生成器只生成脚本，不连接数据库、不写库。部署仍按目标项目的授权和安全门禁执行。角色列不需要逐个录入：默认把真实 `RID` 后的 `bit` 角色列、以及 `SYSWSPACE.admin` 后的权限列全部授权为 `1`，除非契约明确要求例外。`SYS_TbColumn.顺序` 是 `UForm1` 的实际 FlexGrid/字段访问下标，必须在每个 `(表名, PO)` 内从 `0` 开始连续编号为 `0..n-1`；隐藏 identity 主键通常是 `0`。不能使用 `1..n`，也不能保留旧序号（依据见 `client-source-contract.md` 第 9.3.2 节）。
...
4. 新生成元数据的 `类型`/`控件` 按 `client-source-contract.md` 第 9 节生成：物理 `date`/`datetime`/`datetime2` 用 `类型=D`、其余用 `类型=S`，普通字段 `控件=E`。不得按 bit、数值精度或标签自由推导 `C/N/EM/R/RG` 等值；单表基础资料 `标识=NULL`。
5. 新生成可见字段的 `列宽` 按字段 `标签` 的完整显示宽度设置，规则见 `client-source-contract.md` 第 9.6 节（统一使用 `scripts/metadata_width.py`）；显式更宽值可以保留，显式更窄值必须阻断，不能把控件布局 `宽度` 或 `100` 回退值当作列宽。注意 `UForm1` 是**网格表单**：`LoadGridSet` 对 `列宽=0` 会回退到 `100`，所以未显式给宽度的字段会被截断而不是报错。
...
- 新生成字段的 `类型`/`控件` 按 `client-source-contract.md` 第 9 节生成（物理日期 `D`、其余 `S`，普通字段 `E`），不是把 SQL 类型、表体/表头或标签误分成不同默认值。
...
- `BTYPE=1/UForm1` 的 `顺序` 按 `client-source-contract.md` 第 9.3.2 节：每个 `(表名, PO)` 组为 `0..n-1`，无重复、无 NULL；动态单据的 `1..n` 规则不适用于本通道。
